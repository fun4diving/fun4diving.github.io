#!/bin/bash
set -e
echo "🎨 正在升級 Layout 導覽列為極致現代感 (Glassmorphism Floating Nav)..."
...
<html lang="zh-TW" class="dark scroll-smooth">
  ...
  <body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans antialiased selection:bg-cyan-500 selection:text-white relative">
    ...
    <!-- 頂部懸浮玻璃感導覽列 (Floating Glass Nav) -->
    <header class="sticky top-0 z-50 w-full px-3 sm:px-6 pt-3 sm:pt-4 transition-all duration-300">
      <div class="max-w-7xl mx-auto backdrop-blur-xl bg-slate-900/70 border border-slate-700/50 rounded-2xl shadow-2xl shadow-cyan-950/20">
        <div class="flex items-center justify-between h-16 sm:h-20 px-4 sm:px-8">
          
          <!-- Logo 區塊 (帶質感發光效果) -->
          <a href="/fun4diving-website/" class="flex items-center gap-3 group py-1">
            <div class="relative">
              <div class="absolute -inset-1 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 opacity-30 group-hover:opacity-75 blur transition duration-300"></div>
              <img 
                src="https://b1c75e2b05.cbaul-cdnwnd.com/f7e4422cd6db309143b73b292a218cc3/200000014-72a2472a26/tanktest2.png" 
                alt="Fun肆潛水 Logo" 
                class="relative h-10 sm:h-12 w-auto object-contain transition duration-300 transform group-hover:scale-105"
              />
            </div>
            <div class="flex flex-col">
              <span class="font-black text-lg sm:text-xl tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-slate-100 to-cyan-300 group-hover:from-cyan-300 group-hover:to-blue-400 transition">
                Fun肆潛水
              </span>
              <span class="text-[10px] text-cyan-400/80 font-mono tracking-widest uppercase -mt-1 hidden sm:block">
                Fun 4 Diving
              </span>
            </div>
          </a>

          <!-- 桌機選單 (Modern Floating Menu Items) -->
        ...
        <!-- 手機展開式質感抽屜選單 -->
        <div id="mobile-menu" class="hidden lg:hidden border-t border-slate-800/80 px-4 py-4 space-y-1.5 transition-all">
          {navItems.map((item) => {
...
# 推送至 GitHub
echo "🚀 正在將新版高質感導覽列推送至 GitHub..."
git add src/layouts/Layout.astro
git commit -m "Upgrade navigation bar with high-end glassmorphism styling" || true
git push origin main --force

echo "✨ 成功更新！重新整理網頁即可體驗全新質感導覽列！"
