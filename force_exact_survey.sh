...
<Layout title="FUN肆潛水 東北角Fun dive報名表 - FUN 肆潛水">
  <main class="max-w-4xl mx-auto px-4 py-8 text-gray-800">
    <div class="bg-white p-6 sm:p-10 rounded-xl shadow-lg border border-gray-100 space-y-8">
      ...
      <div class="border-b border-gray-200 pb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">FUN肆潛水 東北角Fun dive報名表</h1>
        <div class="text-sm text-gray-600 leading-relaxed space-y-2 bg-blue-50/50 p-4 rounded-lg border border-blue-100">
          <p>集合地點：FUN肆潛水 (新北市瑞芳區東鼎路156號)</p>
          <p>潛點：龍洞/鼻頭角/潮境/陰陽海 (視當日海象決定)</p>
          <p>請詳細填寫以下報名資訊，以利辦理保險與準備裝備！</p>
        </div>
      </div>
      ...
      <form id="full-survey-form" class="space-y-8">
        ...
        <div class="space-y-6">
          ...
          <!-- 1. 報名日期 -->
          <div>
            <label class="block text-sm font-bold mb-1">報名日期 <span class="text-red-500">*</span></label>
          ...
          <div>
            ...
            <div class="space-y-2 text-sm">
              <label class="flex items-center space-x-2"><input type="radio" name="event_type" value="岸潛 Fun dive (兩氣瓶)" required class="text-blue-600" /> <span>岸潛 Fun dive (兩氣瓶)</span></label>
              <label class="flex items-center space-x-2"><input type="radio" name="event_type" value="船潛 Fun dive (兩氣瓶)" class="text-blue-600" /> <span>船潛 Fun dive (兩氣瓶)</span></label>
              <label class="flex items-center space-x-2"><input type="radio" name="event_type" value="夜潛 Fun dive (一氣瓶)" class="text-blue-600" /> <span>夜潛 Fun dive (一氣瓶)</span></label>
            </div>
          </div>

          <!-- 3. 潛水系統與等級 -->
          <div>
            <label class="block text-sm font-bold mb-1">潛水系統與等級 <span class="text-red-500">*</span></label>
            <p class="text-xs text-gray-500 mb-1">例：PADI AOW / SSI OW</p>
            <input type="text" name="certification_detail" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>

          <!-- 4. 氣瓶支數 -->
          <div>
            <label class="block text-sm font-bold mb-1">氣瓶支數 <span class="text-red-500">*</span></label>
            <input type="number" name="logged_dives" required class="w-full sm:w-1/2 border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="請填寫數字" />
          </div>
          ...
          <div>
            ...
            <select name="last_dive_time" required class="w-full sm:w-1/2 border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none">
              ...
              <option value="半年-1年內">半年-1年內</option>
          ...
          <!-- 6. 裝備租借 -->
          <div>
            <label class="block text-sm font-bold mb-2">裝備租借 <span class="text-red-500">*</span></label>
            <div class="space-y-2 text-sm">
              <label class="flex items-center space-x-2"><input type="radio" name="gear_option" value="自備全套裝備" required class="text-blue-600" /> <span>自備全套裝備</span></label>
              <label class="flex items-center space-x-2"><input type="radio" name="gear_option" value="需租借裝備 (請勾選下方租借項目)" class="text-blue-600" /> <span>需租借裝備 (請勾選下方租借項目)</span></label>
            </div>
          </div>

          <!-- 7. 租借項目 (非必填，依需求勾選) -->
          <div>
            <label class="block text-sm font-bold mb-2">租借項目 (需租借裝備者勾選)</label>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="輕裝組 (面鏡/呼吸管/蛙鞋/套鞋)" class="rounded text-blue-600" /> <span>輕裝組 (面鏡/呼吸管/蛙鞋/套鞋)</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="BCD" class="rounded text-blue-600" /> <span>BCD</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="調節器" class="rounded text-blue-600" /> <span>調節器</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="防寒衣" class="rounded text-blue-600" /> <span>防寒衣</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="面鏡" class="rounded text-blue-600" /> <span>面鏡</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="蛙鞋" class="rounded text-blue-600" /> <span>蛙鞋</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="套鞋" class="rounded text-blue-600" /> <span>套鞋</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="電腦錶" class="rounded text-blue-600" /> <span>電腦錶</span></label>
              <label class="flex items-center space-x-2"><input type="checkbox" name="gear_items" value="手電筒" class="rounded text-blue-600" /> <span>手電筒</span></label>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div>
              ...
              <input type="text" name="height" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-gray-700 mb-1">體重 (kg)</label>
              <input type="text" name="weight" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-gray-700 mb-1">腳長 (cm)</label>
              <input type="text" name="shoe_size" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
            </div>
          </div>

          <!-- 9. 備註 / 特別需求 -->
          <div>
            <label class="block text-sm font-bold mb-1">備註 / 特別需求</label>
            <input type="text" name="notes" class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
        </div>

        <!-- ================= 2. PDF 切結書逐字內容區域 ================= -->
        <div class="pt-6 border-t-2 border-gray-200 space-y-6">
          <h2 class="text-xl font-bold text-gray-900">水域遊憩活動特別風險聲明與責任限制切結書</h2>
          <p class="text-xs text-gray-500">(報名線上欄位/紙本簽署專用條款)</p>
          ...
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-300 h-64 overflow-y-scroll text-xs text-gray-700 leading-relaxed space-y-4">
            <p>親愛的活動參與者您好:<br>為了確保您的合法權益及生命財產安全,在您報名參與本潛店(以下簡稱「本公司」)所組織、安排或指導之水域遊憩活動(包括但不限於水肺潛水、船潛、自由潛水、浮潛等)前,請務必詳細閱讀、充分理解並同意本切結書之各項條款。當您勾選同意或簽名時,即視為您已完全理解並願意承擔活動相關風險。</p>
          ...
          <div class="pt-2">
            <label class="flex items-start space-x-2 bg-blue-50 p-3 rounded-lg border border-blue-200 cursor-pointer">
              <input type="checkbox" name="agreed_terms" required class="mt-1 rounded text-blue-600" />
          ...
          <div class="bg-gray-100 p-4 rounded-lg border border-gray-200">
             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="space-y-3 md:pl-4">
                 <div>
                  ...
                  <input type="text" name="emergency_name_relation" required class="w-full border border-gray-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-300">
               <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">法定代理人 (未滿18歲須簽)</label>
                    <input type="text" name="guardian_name" class="w-full border border-gray-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white" />
                 </div>
                 <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">與參與者關係</label>
                    <input type="text" name="guardian_relation" class="w-full border border-gray-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white" />
                 </div>
               </div>
            </div>
          </div>
        ...
        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-lg shadow-lg text-lg transition duration-200 mt-4">
          同意條款並完成電子簽署
        </button>
      </form>
    </div>
