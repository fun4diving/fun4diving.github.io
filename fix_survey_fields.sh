...
echo "⚙️ 更新 src/pages/survey.astro 以完全對齊 PDF 欄位格式..."
...
<Layout title="水域遊憩活動特別風險聲明與責任限制切結書 - FUN 肆潛水">
  <main class="max-w-4xl mx-auto px-4 py-8 text-gray-800">
    <div class="bg-white p-6 sm:p-10 rounded-xl shadow-lg border border-gray-100">
      ...
      <p class="text-xs text-gray-500 text-center mb-8">（報名線上欄位 / 紙本簽署專用條款）</p>
      ...
      <form id="full-survey-form" class="space-y-6">
        ...
        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-lg shadow-md transition duration-200 mt-4">
          同意條款並完成電子簽署
        </button>
      </form>
    </div>
...
git commit -m "Update survey fields to match PDF layout exactly" || true
