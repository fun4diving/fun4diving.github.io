#!/bin/bash
set -e
echo "🎨 正在更新 Layout.astro (導覽列常駐 LINE 按鈕) 與 index.astro (熱血相片牆 + 5星好評)..."

# 1. 更新 Layout.astro - 加入頂部常駐 LINE 諮詢按鈕
...
<html lang="zh-TW" class="scroll-smooth">
  ...
  <body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans antialiased">
    ...
    <!-- 頂部常駐導覽列 -->
    <header class="sticky top-0 z-50 bg-slate-950/85 backdrop-blur-md border-b border-slate-800/80 shadow-2xl">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <a href="/" class="flex items-center gap-2 group">
          ...
          <span class="text-xs px-2 py-0.5 rounded-md bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 hidden sm:inline-block">
            東北角第一店
          </span>
        </a>
        ...
        <nav class="hidden md:flex items-center gap-6 text-sm font-medium text-slate-300">
          <a href="/" class="hover:text-cyan-400 transition">首頁 Index</a>
          <a href="/courses" class="hover:text-cyan-400 transition">課程與住宿 Course</a>
          <a href="/schedule" class="hover:text-cyan-400 transition">潛旅及活動 Schedule</a>
          <a href="http://lin.ee/lSvAOy4" target="_blank" rel="nofollow noreferrer" class="hover:text-cyan-400 transition">聯絡我們 Contact</a>
        </nav>
        ...
        <div class="flex items-center gap-3">
          <a 
            ...
            class="px-4 py-2 sm:px-5 sm:py-2.5 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm flex items-center gap-1.5 shadow-lg shadow-emerald-600/30 transition duration-300 transform hover:scale-105"
          >
            <span>💬 LINE 快速諮詢</span>
          </a>
        </div>
...
# 2. 更新 index.astro - 融入潛友熱血相片牆與 5 星好評區塊
...
<Layout title="首頁 Index :: Fun肆潛水-Fun 4 Diving Official">
  ...
  <section class="relative min-h-[85vh] sm:min-h-[750px] flex items-center justify-center overflow-hidden py-20 px-4 sm:px-6">
    ...
    <div class="absolute inset-0 z-0">
      ...
      />
      <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-slate-950/75"></div>
    </div>
    ...
    <div class="relative z-10 max-w-5xl mx-auto text-center space-y-8">
      ...
      <!-- 號召文案 -->
      <p class="text-white leading-relaxed text-lg sm:text-2xl max-w-3xl mx-auto font-semibold drop-shadow-2xl [text-shadow:_0_2px_12px_rgba(0,0,0,0.95)]">
        東北角Fun Dive第一店！水下沒有喧囂，只有呼吸與湛藍的自由。<br class="hidden sm:inline" />
        嚴守安全底線、玩出熱血極限——準備好跟我們一起探索大海了嗎？
      </p>

      <!-- 雙行動按鈕 -->
      <div class="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
        <a 
          href="/schedule" 
  ...
  <div class="py-16 px-4 sm:px-6 max-w-7xl mx-auto space-y-24">
    ...
    <!-- 2. 潛友熱血相片牆 (Dive Photo Gallery) -->
    <section class="space-y-8">
      <div class="text-center space-y-3">
        <div class="inline-block px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 font-bold text-xs uppercase tracking-wider border border-cyan-500/20">
          Ocean Gallery
        </div>
        <h2 class="text-3xl sm:text-4xl font-black text-white tracking-tight">水下精彩瞬間 ‧ 潛友熱血紀錄</h2>
        <p class="text-slate-400 text-sm max-w-2xl mx-auto">不只是潛水，更是與一群志同道合的夥伴探索無邊大海的熱血記憶</p>
        <div class="w-16 h-1 bg-gradient-to-r from-cyan-400 to-blue-600 mx-auto rounded-full"></div>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- 照片 1 -->
        <div class="group relative h-64 rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
          <img 
            src="https://b1c75e2b05.cbaul-cdnwnd.com/f7e4422cd6db309143b73b292a218cc3/200000008-7b6a07b6a2/S__106807390.jpeg" 
            alt="水下大合照" 
          ...
          <div class="absolute bottom-4 left-4 right-4">
            <span class="px-2.5 py-1 rounded-md bg-cyan-500/80 text-white font-bold text-xs">🤿 熱血團體潛水</span>
            <p class="text-white font-bold text-sm mt-1">湛藍世界裡的無聲歡笑</p>
          </div>
        </div>
        ...
        <div class="group relative h-64 rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent opacity-80"></div>
          <div class="absolute bottom-4 left-4 right-4">
            <span class="px-2.5 py-1 rounded-md bg-blue-500/80 text-white font-bold text-xs">🛥️ 東北角船潛出航</span>
            <p class="text-white font-bold text-sm mt-1">解鎖私房潛點秘境</p>
          </div>
        </div>
        ...
        <div class="group relative h-64 rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent opacity-80"></div>
          <div class="absolute bottom-4 left-4 right-4">
            <span class="px-2.5 py-1 rounded-md bg-emerald-500/80 text-white font-bold text-xs">🌊 岸潛教學與探險</span>
            <p class="text-white font-bold text-sm mt-1">嚴守安全底線、細心帶領</p>
          </div>
        </div>
        ...
        <div class="group relative h-64 rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent opacity-80"></div>
          <div class="absolute bottom-4 left-4 right-4">
            <span class="px-2.5 py-1 rounded-md bg-indigo-500/80 text-white font-bold text-xs">✈️ 國內外夢幻潛旅</span>
            <p class="text-white font-bold text-sm mt-1">小琉球 ‧ 綠島 ‧ 帛琉 ‧ 西巴丹</p>
          </div>
        </div>
      </div>
    </section>

    <!-- 3. 潛友 5 星好評區塊 (Student Testimonials) -->
    <section class="space-y-8 bg-slate-900/40 p-8 sm:p-12 rounded-3xl border border-white/5 shadow-2xl">
      <div class="text-center space-y-3">
        <div class="inline-flex items-center gap-1 text-amber-400 font-bold text-base">
          <span>★</span><span>★</span><span>★</span><span>★</span><span>★</span>
          <span class="text-slate-300 text-sm ml-2">5.0 潛友高分滿意度</span>
        </div>
        <h2 class="text-3xl sm:text-4xl font-black text-white tracking-tight">潛友真心推薦 ‧ 選擇 Fun肆 的理由</h2>
        <div class="w-16 h-1 bg-gradient-to-r from-cyan-400 to-blue-600 mx-auto rounded-full"></div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        <!-- 好評 1 -->
        <div class="bg-slate-900/90 rounded-2xl p-6 border border-slate-800 space-y-4 shadow-xl flex flex-col justify-between hover:border-cyan-500/30 transition">
          <div class="space-y-3">
            <div class="text-amber-400 text-sm">★★★★★</div>
            <p class="text-slate-300 text-sm leading-relaxed italic">
              「導潛教練專業又超親切！水下把安全顧得非常仔細，帶我們看的潛點生態超豐富，跟著 Fun 肆下水安全感滿滿！」
            </p>
          </div>
          <div class="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
            <span class="font-bold text-white">🤿 潛友 Ken</span>
            <span class="text-cyan-400">PADI AOW 潛水員</span>
          </div>
        </div>
        ...
        <div class="bg-slate-900/90 rounded-2xl p-6 border border-slate-800 space-y-4 shadow-xl flex flex-col justify-between hover:border-cyan-500/30 transition">
          <div class="space-y-3">
            <div class="text-amber-400 text-sm">★★★★★</div>
            <p class="text-slate-300 text-sm leading-relaxed italic">
              「Fun Dive 行程安排得太棒了，氣氛歡樂完全沒有壓力！而且背包客房乾淨舒服，週五晚上先住下來隔天輕鬆下海超方便！」
            </p>
          </div>
          <div class="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
            <span class="font-bold text-white">🌊 潛友 咪咪</span>
            <span class="text-cyan-400">東北角船潛團員</span>
          </div>
        </div>
        ...
        <div class="bg-slate-900/90 rounded-2xl p-6 border border-slate-800 space-y-4 shadow-xl flex flex-col justify-between hover:border-cyan-500/30 transition">
          <div class="space-y-3">
            <div class="text-amber-400 text-sm">★★★★★</div>
            <p class="text-slate-300 text-sm leading-relaxed italic">
              「本來很怕水，但在 Fun肆 教練細心的一對一指導下順利拿到 OW 證照！加入 Line 社群還結交了一群熱血潛友，推薦大家首選 Fun肆！」
            </p>
          </div>
          <div class="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
            <span class="font-bold text-white">✨ 潛友 Anson</span>
            <span class="text-cyan-400">OW 課程學員</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="space-y-12">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl flex flex-col justify-between">
          ...
          <div class="p-8 text-center space-y-4">
            <h3 class="text-2xl font-bold text-white">潛旅及活動行程</h3>
...
# Git 提交並自動推送至 GitHub
git add . && git commit -m "Add sticky LINE navbar button, photo gallery, and 5-star testimonials section" && git push origin main

echo "✨ 升級完成！常駐 LINE 按鈕、熱血相片牆與 5 星好評區塊皆已更新並自動推送發布上線！"
