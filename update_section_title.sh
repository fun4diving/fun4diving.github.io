#!/bin/bash
set -e
echo "📝 正在更新 index.astro：將『Fun Dive行程』改名為『潛旅及活動行程』..."
...
<Layout title="首頁 Index :: Fun肆潛水-Fun 4 Diving Official">
  ...
  <section class="relative min-h-[85vh] sm:min-h-[750px] flex items-center justify-center overflow-hidden py-20 px-4 sm:px-6">
    ...
    <div class="relative z-10 max-w-5xl mx-auto text-center space-y-8">
      ...
      <!-- 4. 四大品質認證卡片 -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto pt-2">
        <div class="bg-slate-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/10 text-center space-y-1 shadow-xl hover:border-cyan-400/50 transition">
          <div class="text-cyan-400 font-bold text-base sm:text-lg">🤿 專業導潛及教練</div>
        ...
        <div class="bg-slate-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/10 text-center space-y-1 shadow-xl hover:border-cyan-400/50 transition">
          <div class="text-cyan-400 font-bold text-base sm:text-lg">🛡️ 合法立案投保</div>
          <div class="text-slate-300 text-xs">全程投保安心水下保障</div>
        </div>
      </div>
...
# Git 提交並自動推送至 GitHub
git add . && git commit -m "Rename Fun Dive card title to '潛旅及活動行程' on index page" && git push origin main

echo "✨ 修改完成！卡片標題已成功改為『潛旅及活動行程』並自動發布上線！"
