#!/bin/bash
set -e
echo "📝 正在更新 schedule.astro：採用 SVG 國旗圖標（含台灣國旗 🇹🇼），確保 Chrome 完全正常顯示..."
...
git commit -m "Fix country flags rendering using high-res SVG vectors and add Taiwan flag for domestic trips" || true
git push origin main --force

echo "✨ 完成！現在跨平台/跨瀏覽器（含 Chrome）都可以看到精美的國旗了！"
