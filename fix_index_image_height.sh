#!/bin/bash
set -e
echo "🎨 正在更新首頁：拉大圖片框高度，讓下半部照片完整呈現..."
...
<Layout title="Fun肆潛水-Fun 4 Diving Official :: 首頁 Index">
  ...
  <!-- 3. 課程及行程詳細內容區塊 (把照片框由 h-64 拉大至 h-96，讓照片下半部完整露出) -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        ...
        >
          <div class="relative h-80 sm:h-96 overflow-hidden">
            ...
            />
            <!-- 薄漸層遮罩 -->
            <div class="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent"></div>
          </div>
          <div class="p-8 space-y-2 text-center flex-grow flex flex-col justify-center relative z-10">
...
# 推送至 GitHub
echo "🚀 正在將加大圖片高度的更新推送至 GitHub..."
git add src/pages/index.astro
git commit -m "Increase card image height to h-96 for fuller photo display" || true
git push origin main --force

echo "✨ 成功修復！照片框已向下拉大，照片下半部可以完整看得清清楚楚！"
