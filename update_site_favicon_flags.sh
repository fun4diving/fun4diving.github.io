...
echo "📝 1. 正在確認 Layout.astro 的 Favicon 設定..."
# 確保 Layout.astro 引用 /favicon.png
if [ -f "src/layouts/Layout.astro" ]; then
  sed -i 's|href="/favicon.svg"|href="/favicon.png"|g' src/layouts/Layout.astro || true
  sed -i 's|href="/favicon.ico"|href="/favicon.png"|g' src/layouts/Layout.astro || true
fi

echo "📝 2. 正在更新 schedule.astro (確保小國旗完整保留)..."
...
# Git commit & Push
git add .
git commit -m "Keep country flags & update layout favicon" || true
