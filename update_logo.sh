#!/bin/bash
set -e
echo "🎨 正在更新 Logo：移除文字，僅保留純圖片書法 Logo..."
...
<html lang="zh-TW" class="dark scroll-smooth">
  ...
  <body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans antialiased selection:bg-cyan-500 selection:text-white relative">
    ...
    <header class="sticky top-0 z-50 w-full px-3 sm:px-6 pt-3 sm:pt-4 transition-all duration-300">
      <div class="max-w-7xl mx-auto backdrop-blur-xl bg-slate-900/80 border border-slate-700/50 rounded-2xl shadow-2xl shadow-cyan-950/20">
        <div class="flex items-center justify-between h-16 sm:h-20 px-4 sm:px-8">
          
          <!-- LOGO 區塊：只保留純圖片，移除所有旁邊的文字 -->
          <a href="/fun4diving-website/" class="flex items-center group py-1">
            <img 
              src="/fun4diving-website/logowhite.png" 
              alt="Fun肆潛水 Logo" 
              class="h-9 sm:h-11 w-auto object-contain transition duration-300 transform group-hover:scale-105"
            />
          </a>
...
git commit -m "Replace logo with pure white calligraphy image" || true
git push origin main --force

echo "✨ 成功替換！頂部已改成純書法圖片 LOGO！"
