#!/bin/bash
set -e
echo "🎨 正在更新 src/pages/schedule.astro，將外框卡片風格與其他分頁完全統一..."
...
<Layout title="Fun肆潛水活動列表 Schedule :: Fun肆潛水-Fun 4 Diving Official">
  <section class="max-w-6xl mx-auto py-12 px-4 sm:px-6">
    ...
    <!-- 主內容區：雙欄佈局 (與其他分頁一致的質感卡片外框) -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      
      <!-- 左欄：Fun肆經典活動 -->
      <div class="bg-slate-900/80 p-6 sm:p-8 rounded-2xl border border-slate-800 shadow-xl space-y-8 h-fit">
        <h2 class="text-2xl font-black text-cyan-400 border-b border-slate-800 pb-4 flex items-center gap-2">
          <span>🎉</span> Fun肆經典活動
        </h2>

        <!-- 年終感恩會 -->
        <div class="space-y-2 p-4 bg-slate-950/60 rounded-xl border border-slate-800/80">
      ...
      <div class="space-y-8">
        ...
        <!-- 國內潛水活動 -->
        <div class="bg-slate-900/80 p-6 sm:p-8 rounded-2xl border border-slate-800 shadow-xl space-y-6">
          <h2 class="text-2xl font-black text-cyan-400 border-b border-slate-800 pb-4 flex items-center gap-2">
        ...
        <div class="bg-slate-900/80 p-6 sm:p-8 rounded-2xl border border-slate-800 shadow-xl space-y-6">
          ...
          <div class="space-y-3 pb-4 border-b border-slate-800/60">
            ...
            <div class="rounded-xl overflow-hidden border border-slate-800 max-h-56">
...
# 推送至 GitHub
echo "🚀 正在將更新後的 Schedule 頁面推送至 GitHub..."
git add src/pages/schedule.astro
git commit -m "Unify Schedule page card styling with other pages" || true
git push origin main --force

echo "✨ 成功修復！「活動列表 Schedule」頁面的卡片外框已與其他分頁一致！"
